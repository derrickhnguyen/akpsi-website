...
function highlightNav() {
	var num;
   	...
   	$('.navbar-nav li a').eq(num).css('background-color', '#f1f1f1');
}

// This function generates the parallax background for each page and the appropraite parallax effect.
// If the user is viewing the website via a mobile device, a different parallax image will
...
// This function will change the padding of the parallax text depending
...
// the screen will go all the way to the top page.
(function backToTop() {
...
$(document).ready(function() {
	// Highlights section in navigation that user is in.
	highlightNav();

	// Generates parallax background images.
	parallaxGenerator();

	// Generate padding of parallax.
	coverParallax();
	...
	if(window.location.pathname === '/') {
		...
		$nav.css('display', 'none').fadeIn(350);
	} 

	// Change the collor of parallax text to white.
	$('.parallax').css('color', 'white');

	// Make the navigation bar fixed to the top of the page
